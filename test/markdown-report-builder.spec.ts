...
describe('MarkdownReportBuilder', () => {
    ...
    test('Markdown with multiple reports with no errors', async () => {
        ...
        await MarkdownReportBuilder.buildReports(reports, testResultFile);
        const createdReport = replaceWhitespaces(
            await Promise.resolve(Bun.file(`${testResultFile}.md`).text())
        );
        const expectedReport = replaceWhitespaces(
            await Promise.resolve(Bun.file(`${expectedResultDir}testreport_noerror.md`).text())
        );
    ...
    test('JSON with multiple reports with errors', async () => {
        ...
        const expectedReport = replaceWhitespaces(
            await Promise.resolve(Bun.file(`${expectedResultDir}testreport_witherrors.md`).text())
        );
